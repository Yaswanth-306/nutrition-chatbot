"""
ML API with Dr. Majidi's Exact Clinical Thresholds
Based on Presentation Slides 8, 10, and 12
"""
...
# === EXACT THRESHOLDS FROM DR. MAJIDI'S PRESENTATION ===

# Slide 8: BMI Categories
BMI_THRESHOLDS = {
    'underweight': 18.5,
    'normal': 25,
    'overweight': 30,
...
# Load model
try:
    model = joblib.load('models/trained_model_majidi.pkl')
    with open('models/feature_names_majidi.txt', 'r') as f:
...
def categorize_bmi(bmi):
    """Categorize BMI based on Slide 8"""
...
def categorize_fbs(fbs):
    """Categorize FBS based on Slide 12 (ADA Guidelines)"""
...
def categorize_hdl(hdl, sex):
    """Categorize HDL based on Slide 10 (Sex-specific)"""
    if sex.lower() == 'male':
...
def categorize_ldl(ldl):
    """Categorize LDL based on Slide 10"""
...
def categorize_tg(tg):
    """Categorize Triglycerides based on Slide 10"""
...
def calculate_metabolic_syndrome_score(fbs, tg, hdl, bmi, sex):
    """Calculate metabolic syndrome risk score (0-4)"""
    score = 0
...
def predict_risk():
    """Predict glycemic risk using Dr. Majidi's exact thresholds"""
    try:
        data = request.json
...
if __name__ == '__main__':
    print("🚀 Dr. Majidi's ML API Server (Exact Thresholds)")
    print("📊 Based on Presentation Slides 8, 10, 12")
    print("="*60)
